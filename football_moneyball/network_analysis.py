"""
Modulo de analise de redes de passes a partir de dados StatsBomb.
...
def build_pass_network(
    match_id: int, team: str = None
) -> tuple[nx.DiGraph, pd.DataFrame]:
    """
    Constroi um grafo direcionado de rede de passes a partir dos eventos
    de uma partida StatsBomb.
...
def get_network_from_db(
    session, match_id: int, team: str = None
) -> nx.DiGraph:
    """
    Reconstroi o grafo de rede de passes a partir da tabela pass_networks
    no banco de dados.

    Parametros
    ----------
    session : sqlalchemy.orm.Session
        Sessao ativa do SQLAlchemy.
    match_id : int
    ...
    -------
    nx.DiGraph
        Grafo direcionado reconstruido a partir dos registros do banco.
    """
    from football_moneyball.db import PassNetwork

    query = session.query(PassNetwork).filter(
        PassNetwork.match_id == match_id
    )
    ...
    G = nx.DiGraph()
    for row in rows:
        if not G.has_node(row.passer_id):
            G.add_node(
                row.passer_id,
                player_name=row.passer_name or str(row.passer_id),
            )
        if not G.has_node(row.receiver_id):
            G.add_node(
                row.receiver_id,
                player_name=row.receiver_name or str(row.receiver_id),
            )

        edge_attrs = {"weight": row.weight or 0}
        if row.features is not None:
            edge_attrs.update(row.features)

        G.add_edge(row.passer_id, row.receiver_id, **edge_attrs)

    # Se team foi fornecido, filtrar por nomes de jogadores no grafo
    # (a tabela pass_networks nao tem coluna de time)
    return G
