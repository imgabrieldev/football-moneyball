...
from football_moneyball.domain.constants import (
    ...
    HIGH_TURNOVER_X_THRESHOLD,
    PRESSING_RECOVERY_WINDOW,
    PRESSING_ZONE_BOUNDARIES,
    SHOT_AFTER_TURNOVER_WINDOW,
)
...
def _compute_ppda(events: pd.DataFrame, team: str, opponent: str) -> float:
    ...
    def_actions = events[
        (events["type"].isin(DEFENSIVE_ACTION_TYPES))
        & (events["team"] == team)
    ]
...
def _compute_pressing_success(events: pd.DataFrame, team: str) -> float:
    ...
    for p_ts in pressure_ts:
        ...
        if np.any((time_diffs >= 0) & (time_diffs <= PRESSING_RECOVERY_WINDOW)):
            successful += 1
...
def _compute_high_turnovers(
    ...
    high_recoveries = recoveries[rec_x >= HIGH_TURNOVER_X_THRESHOLD]
    ...
    for idx in high_recoveries.index:
        ...
        if np.any((time_diffs >= 0) & (time_diffs <= SHOT_AFTER_TURNOVER_WINDOW)):
            n_shot_ending += 1
...
def _compute_pressing_zones(events: pd.DataFrame, team: str) -> list[float]:
    ...
    total = len(pressures)
    zones = []
    for low, high in PRESSING_ZONE_BOUNDARIES:
