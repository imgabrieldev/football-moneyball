"""Testes para player_metrics.py — metricas expandidas v0.2.0."""
...
from football_moneyball.player_metrics import (
    POSITION_GROUP_MAP,
...
class TestExtractPlayerPositions:
    @patch("football_moneyball.player_metrics.sb")
    def test_extracts_positions_from_lineups(self, mock_sb, mock_lineups):
        mock_sb.lineups.return_value = mock_lineups
        positions = extract_player_positions(match_id=12345)
...
class TestExtractMatchMetrics:
    @patch("football_moneyball.player_metrics.sb")
    def test_pass_breakdown(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
    ...
    @patch("football_moneyball.player_metrics.sb")
    def test_big_chances(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
    ...
    @patch("football_moneyball.player_metrics.sb")
    def test_ground_duels(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
    ...
    @patch("football_moneyball.player_metrics.sb")
    def test_passes_under_pressure(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
    ...
    @patch("football_moneyball.player_metrics.sb")
    def test_switches_of_play(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
    ...
    @patch("football_moneyball.player_metrics.sb")
    def test_all_new_columns_present(self, mock_sb, mock_events_df):
        mock_sb.events.return_value = mock_events_df
        df = extract_match_metrics(match_id=1)
