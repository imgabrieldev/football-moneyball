"""Testes para player_embeddings.py — position-aware embeddings v0.2.0."""

import numpy as np
import pandas as pd
import pytest

from football_moneyball.player_embeddings import (
    generate_embeddings,
    cluster_players,
    _find_optimal_k,
    _GROUP_ARCHETYPES,
...
class TestGroupArchetypes:
    def test_has_all_position_groups(self):
        for group in ["DEF", "MID", "FWD", "GK"]:
            assert group in _GROUP_ARCHETYPES

    def test_each_group_has_archetypes(self):
        for group, archetypes in _GROUP_ARCHETYPES.items():
